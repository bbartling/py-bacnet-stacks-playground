"""Data-contract audit tests (quality / columns / topology)."""
...
def test_columns_intersect_warns_on_missing_history_cols(tmp_path: Path):
    ...
    warnings, present = audit_columns_vs_history("AHU_1", df, cols)
...
def test_quality_trusted_after_data_end_warns_keeps_rows():
    ...
    warnings = audit_quality_window("VAV_1", df, quality)
...
def test_vav_map_loads(tmp_path: Path):
    ...
    # function expects building root containing the csv
    root = tmp_path
