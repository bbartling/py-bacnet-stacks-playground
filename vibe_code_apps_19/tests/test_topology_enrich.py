...
def test_enrich_copies_parent_sat():
    idx = _idx()
    ahu = pd.DataFrame({"sat": [55.0] * 6}, index=idx)
    ahu.attrs["equipment_type"] = "AHU"
    vav = pd.DataFrame(
        {"vav_disch_t": [55.0, 55.0, 70.0, 70.0, 70.0, 70.0], "zone_flow": [100.0] * 6},
        index=idx,
    )
...
def test_stamp_feed_attrs():
    idx = _idx(2)
    ahu = pd.DataFrame({"sat": [55.0, 55.0]}, index=idx)
    ahu.attrs["equipment_type"] = "AHU"
    vav = pd.DataFrame({"vav_disch_t": [55.0, 55.0]}, index=idx)
    vav.attrs["equipment_type"] = "VAV"
    frames = {"AHU_1": ahu, "VAV_1": vav}
...
def test_vav_ahu_leave_skipped_without_ahu_sat():
    idx = _idx()
    df = pd.DataFrame(
        {"vav_disch_t": [70.0] * 6, "zone_flow": [100.0] * 6},
        index=idx,
    )
    ...
    assert "ahu_sat" in (r.missing_roles or [])
