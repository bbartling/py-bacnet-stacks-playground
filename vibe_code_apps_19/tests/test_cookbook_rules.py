...
def test_inventory_metadata():
    ...
    assert inv["canonical_rule_count"] == 53
    assert len(inv["rules"]) == 53
...
def test_run_all_returns_active_catalog():
    ...
    assert sum(1 for r in results if not str(r.rule_id).startswith("CUSTOM-")) == 53
