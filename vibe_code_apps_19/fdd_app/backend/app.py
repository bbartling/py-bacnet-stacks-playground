...
Run:
  cd fdd_app && uvicorn asgi:app --host 127.0.0.0 --port 5000
  cd fdd_app/backend && python app.py
  gunicorn -k uvicorn.workers.UvicornWorker --timeout 300 asgi:app  # from fdd_app/
"""
...
for _p in (BACKEND, FDD_APP / "sidecar"):
    _s = str(_p)
...
STATIC_DIR = FDD_APP / "frontend" / "static"
DATA_DIR = BACKEND / "data"
NOTES_FILE = DATA_DIR / "analyst_notes.json"
SESSION_FILE = BACKEND / "analyst_session.json"
...
def _register_full_routes(app: FastAPI, *, html_shell: bool = True) -> None:
    ...
    @app.get("/api/historian/export")
    def api_historian_export_status() -> JSONResponse:
        ...
        return JSONResponse({
            "ok": True,
            "needs_export": he.needs_export(),
            "export_dir": str(dest),
            "historian_subdir": he.historian_subdir(),
            "meta": meta,
        })

    @app.post("/api/historian/export")
    def api_historian_export_run(body: dict = Body(default={})) -> JSONResponse:
        import historian_export as he

        equipment_ids = body.get("equipment_ids")
        try:
            meta = he.export_all(equipment_ids=equipment_ids)
        except Exception as exc:  # noqa: BLE001
            return JSONResponse({"ok": False, "error": f"{type(exc).__name__}: {exc}"}, status_code=500)
        return JSONResponse(meta)

    @app.get("/api/sidecar/status")
    def api_sidecar_status() -> JSONResponse:
        ...
        return JSONResponse({
            "ok": True,
            "edge_url": cs.edge_url(),
            "openfdd_available": cs.is_available(),
            "rust_fdd": rfb.status(),
            "sql_rules": sorted(csql.sql_catalog().keys()),
        })
    ...
    def _startup_warmup() -> None:
        ...
        def _bg() -> None:
            ...
            try:
                ...
                if os.environ.get("OPENFDD_AUTO_EXPORT", "0") == "1":
                    import historian_export as he
                    if he.needs_export():
                        he.export_all()
