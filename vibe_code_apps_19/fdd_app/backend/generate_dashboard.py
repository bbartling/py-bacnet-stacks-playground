...
def compute_mech_cool_oat_bins(plant: dict, wx: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        from duckdb_rollups import chiller_oat_bin_hours
        out = chiller_oat_bin_hours(plant, wx, poll_seconds=POLL_SECONDS)
        if not out.empty:
            return out
    except Exception:
        pass
...
def body_index(ctx: dict) -> str:
    ...
    ck = ctx.get("cookbook_kpis") or {}
    ck_unocc = ck.get("excess_fan_fault_h")
    ck_note = ""
    if ck_unocc is not None:
        ck_note = f' <span class="note">(cookbook SCHED-1: {ck_unocc:.0f} h)</span>'
...
<div class="grid">
  ...
  <div class="kpi"><div class="val">{unocc:.0f}</div><div class="lbl">Excess unoccupied fan hours{ck_note}</div></div>
</div>"""
    return kpis
...
def _compute_full_context(raw: dict) -> dict:
    ...
    cookbook_kpis: dict = {}
    try:
        import cookbook_kpi as ck
        cookbook_kpis = ck.overview_kpis()
    except Exception:
        pass
    return {
        "ahu1": ahu1,
...
def compute_context(raw: dict, page_id: str | None = None) -> dict:
    ...
    if page_id == "motor_runtime":
        from motor_runtime_cache import compute_all_motor_stats

        motor_rows = compute_all_motor_stats(raw)
