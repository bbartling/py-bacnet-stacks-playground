...
17. **Agent → Streamlit**: after `agent_afdd.py --run-all`, open http://localhost:8501 — `.last_agent_session.json` / `VIBE19_BOOTSTRAP` auto-loads package + dialed params **locally**. On Cloud/GHCR: download/upload `session_config.json` (sidebar); default agent file is **not** auto-ingested unless `VIBE19_BOOTSTRAP` is set. **Browser zip uploads** are per Streamlit session (`app/session_workspace.py`). Local refresh restore via `.last_browser_session.json` is disabled in cloud mode; set `VIBE19_BROWSER_AUTOLOAD=0` in AppTest/CI.
