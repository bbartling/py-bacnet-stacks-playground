...
12. **Runtime proof** — motor/pump/DX status first for weekly motor charts. No pump in data model → **omit** that motor series (never invent motor hours from leave temp). Prefer mapped fan/pump roles over column-name invent.
13. **Mech-cooling OAT bins = mechanical compressors / plant only** — chiller plant **or** AHU / heat pump / RTU with **DX / compressor** roles. Sidebar checkbox **Use mapped mechanical-cooling status proof** (default checked): pump → chiller status → amps → power. Unchecked: CHW plants use **CHW leave proof max °F**, labeled `inferred: chw_leave_temp` (slider disabled while status proof checked). Always show every cooling-capable device in Overview coverage with name / included|excluded / proof / runtime hours / reason. Aggregate = **device-hours**. **Never** use `clg_valve_pct` / CHW cooling-valve %. `include_ahu_chw_valve` stays deprecated/ignored. Sort bins by `bin_start` cold→hot. **WattLab dump** always calls `run_rules` for a complete cookbook (never reuse partial `batch_results`).
