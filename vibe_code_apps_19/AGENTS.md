...
10. **Runtime proof = motor/status first (default)** — weekly motor charts use designated `chw_pump_status` / `chw_pump_cmd` (or DX compressor on AHU/HP/RTU). No pump/status → **omit** that series from motor charts (never invent motor hours from leave temp). Mech-cooling OAT bins default to the same hard-status ladder; an explicit sidebar checkbox can switch CHW plants to inferred leave-temp (see rule 11). Motor charts prefer mapped fan/pump roles before column-name heuristics.
11. **Mech-cooling OAT bins = mechanical compressors / plant only** — chiller plant **or** AHU / heat pump / RTU with **DX / compressor** stages. Sidebar **Use mapped mechanical-cooling status proof** (default checked): pump → chiller status → amps → power. Unchecked: CHW plants use **CHW leave proof max °F** labeled `inferred: chw_leave_temp` (slider disabled while status proof is checked). Always show every cooling-capable device in Overview coverage (name, included/excluded, proof, runtime hours, reason). Aggregate remains **device-hours**. **Never** treat AHU `clg_valve_pct` / CHW cooling-valve % as mechanical cooling. `include_ahu_chw_valve` stays deprecated/ignored. Bins sort cold→hot by `bin_start`. **WattLab dump** always re-runs the complete cookbook (never reuse partial `batch_results`).
...
- **Chiller plant runtime** on weekly motor charts: designated CHW **pump status** only. No pump in the data model → **no series** (never fake motor hours from leave temp).
- **Mech-cooling OAT bins** (`app/analytics.py`): compressor devices only. Default hard-status proof (`chw_pump_*` → `chiller_status` → amps/power); optional inferred CHW leave temp when `use_mech_cooling_status_proof` is false. Never `clg_valve_pct` / cooling valve. Session key `include_ahu_chw_valve` is deprecated/ignored. Coverage table always lists every cooling-capable device.
