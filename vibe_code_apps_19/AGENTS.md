...
On Streamlit start (empty session), **local** mode **auto-loads** that package + fault settings and optionally **re-runs all 50 rules** so Plots/RCx are ready at **http://localhost:8501**. Cloud/GHCR skips `.last_agent_session.json` unless `VIBE19_BOOTSTRAP` is set.
