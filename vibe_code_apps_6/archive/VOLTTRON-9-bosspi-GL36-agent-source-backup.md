# GL36-ish VOLTTRON agent source backup

This is a Windows-side backup of the new GL36-ish VOLTTRON agent source that was installed on the Raspberry Pi bench host.
...
- This is a **safe first-pass supervisory implementation**, not a full site-tuned Guideline 36 package.
- The AHU agent runs in **publish-only recommendation mode** by default.
- Because the bench currently has only one VAV, the aggregation logic is more important than the raw totals observed so far.
- Keep this file as a Windows-side backup for future OpenClaw sessions even if the Pi source changes later.
