"""
Merge per-device point CSVs into one commissioning file (for read driver / points_discovered).

  python -m edge_bacnet.merge_points_csv \\
    --input-dir edge_backup/local/acme/vm-bbartling/points_per_device \\
    -o edge_backup/local/acme/vm-bbartling/points_discovered.csv
"""
...
def merge_points_dir(
    ...
    *,
    ...
    sys.stderr.write(
        f"Merged {len(rows)} rows from {len(files)} file(s) → {output}\n"
    )
    return 0
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory containing device_<instance>.csv files",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        type=Path,
        help="Combined output CSV (e.g. points_discovered.csv or points.csv)",
    )
    parser.add_argument(
        "--enabled-only",
        action="store_true",
        help="Only include rows with enabled=1 (for final points.csv)",
    )
    parser.add_argument(
        "--pattern",
        default="device_*.csv",
        help="Glob under input-dir (default: device_*.csv)",
    )
