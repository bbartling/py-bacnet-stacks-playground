...
Read BACnet points for each device listed in devices_discovered.csv.
  ...
  python -m edge_bacnet.discover_points \\
    --from-devices devices_discovered.csv \\
    -o points_discovered.csv \\
    --name GatewayVmBbartling --instance 3456791 \\
    --address 10.200.200.185/24:47808
"""
...
from edge_bacnet.discover_lib import object_identifiers, read_point_props, set_show_warnings, write_csv
...
def _build_parser() -> SimpleArgumentParser:
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="points_discovered.csv",
        help="Combined CSV (optional if --per-device-dir only)",
    )
    parser.add_argument(
        "--per-device-dir",
        default=None,
