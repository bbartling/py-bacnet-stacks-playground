...
BUILDING = "tower-a"
MOCK_POINT = {
    "series_id": f"{SITE}#{BUILDING}#ahu-1#sat",
...
class TestReadingsPayload(unittest.TestCase):
    ...
    @unittest.mock.patch.object(
        lambda_function,
        "_fetch_building_readings",
        return_value=(_sample_readings(), MOCK_POINT),
    )
    ...
    def test_includes_fault_analytics(
        ...
        payload = lambda_function._readings_payload(
            SITE, BUILDING, 2, rolling_avg_minutes=1
        )
    ...
    @unittest.mock.patch.object(
        lambda_function,
        "_fetch_building_readings",
        return_value=(_sample_readings(8100, step_ms=10_000), MOCK_POINT),
    )
    @unittest.mock.patch.object(lambda_function, "_load_custom_rules")
    def test_chunked_path_builds_fault_analytics(
        ...
        payload = lambda_function._readings_payload(
            SITE, BUILDING, 24, rolling_avg_minutes=1
        )
