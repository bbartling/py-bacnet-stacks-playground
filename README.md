...
| **11** | **AI spec-driven BAS**                                                                       | **OpenClaw-style experiment** in `vibe_code_apps_11/`: `bas_build_spec/` (product spec, `SKILL.md` skills, memory, cron) + scheduled **Codex** wakes build **`bas_app/`** from acceptance criteria. Electrical phase: **Who-Is every 5 min** (workers), **Codex every 3 h**, public **rough-in** commissioning UI/chat, auto wire via `PHASE_NOTEPAD.md` — not a Django/React production milestone. | **Active**  |
| **12** | **RPi BACnet sensor + AWS IoT**                                                              | **Edge lab** in `vibe_code_apps_12/`: DS18B20 on **GPIO / 1-Wire** → local **BACnet/IP** device (BACpypes3-style server) + optional **AWS IoT Core** publish over **MQTT/TLS**; working BACnet server patterns for bench and Pi deploy (see app README + `ansible/`).                                                                                                                                     | **Active**  |
...
| **15** | **Rust BACnet stack**                                                                        | **Future vibe app** (`vibe_code_apps_15/`): integrate [`rusty-bacnet`](https://github.com/jscott3201/rusty-bacnet) with Python bindings for performance-sensitive BACnet/IP paths, benchmarks, and optional bridge experiments alongside BACpypes3.                                                                                     | Planned     |
