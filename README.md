...
| **11** | **[AI spec-driven BAS](vibe_code_apps_11/)** (`vibe_code_apps_11`)                         | **OpenClaw-style experiment:** `bas_build_spec/` (product spec, `SKILL.md` skills, memory, cron) + scheduled **Codex** wakes build **`bas_app/`** from acceptance criteria. Electrical phase: **Who-Is every 5 min** (workers), **Codex every 3 h**, public **rough-in** commissioning UI/chat, auto wire via `PHASE_NOTEPAD.md` — not a Django/React production milestone. | **Active**  |
| **12** | **[RPi BACnet sensor + AWS IoT](vibe_code_apps_12/)** (`vibe_code_apps_12`)                | **Edge lab:** DS18B20 on **GPIO / 1-Wire** → local **BACnet/IP** device (BACpypes3-style server) + optional **AWS IoT Core** publish over **MQTT/TLS**; working BACnet server patterns for bench and Pi deploy (see app README + `ansible/`).                                                                                                                                     | **Active**  |
...
| **15** | **[Rust BACnet stack](vibe_code_apps_15/)** (`vibe_code_apps_15`, planned)                   | **Future vibe app:** integrate [`rusty-bacnet`](https://github.com/jscott3201/rusty-bacnet) with Python bindings for performance-sensitive BACnet/IP paths, benchmarks, and optional bridge experiments alongside BACpypes3.                                                                                     | Planned     |
