...
**Active development** is in **`vibe_code_apps_12/`** (edge BACnet + cloud FDD pipeline), **`vibe_code_apps_14/`** (BACnet routing research), and **`vibe_code_apps_11/`** (agentic BAS — paused, revisit later). Checkpoints **1–10** are historical demos; **13** and **15–16** are planned future apps (`vibe_code_apps_13/` is a reserved slot).
...
| **11** | **Agentic BAS from spec**                                                                    | **Paused (revisit later).** Agentic software experiment: **SKILL.md** specifications, workspace memory, and Linux **cron** jobs drive a custom **OpenAI Codex CLI** agent that can generate and maintain its own BAS codebase—supervisory web app, BACnet drivers, commissioning flows, and related automation. Goal is spec-driven, self-updating BAS software rather than hand-writing every module. Folder: `vibe_code_apps_11`.                                                                                                                                                                                                 | Paused      |
| **12** | **Edge BACnet + cloud FDD (full pipeline)**                                                  | **Active.** End-to-end **fault detection & diagnostics (FDD)** stack for real buildings—not just a demo sensor. **Edge (Linux/Pi):** BACnet **Who-Is discovery** → commissioning CSV → **RPM read driver** → **AWS IoT Core** MQTT (`vibe12/{site}/{building}/{system}/{point}/telemetry`); Ansible defaults to **BACnet-only** (GPIO/DS18B20 opt-in for the lab bench). **Cloud:** IoT rules → **DynamoDB** historian → Lambda **dashboard** + **Bake-a-Py Rule Lab** (Python FDD rules). **Data model:** BRICK-style `{sites, equipment, points}`, TTL projection, health checks, AI-assisted JSON import. **FDD:** zone-temperature rule packs, multi-series/mechanical recipes, **BRICK-class-scoped** rules across many sensors. Folder: `vibe_code_apps_12`. | **Active**  |
| **13** | **DIY BACnet router (planned)**                                                              | **Future vibe app:** integrated **BACnet/IP ↔ MS/TP** router on Linux (e.g. Raspberry Pi + USB RS-485) — ship-ready path using **bacnet-stack** `router-mstp`, informed by Misty3 / BACpypes3 experiments in app **14**. Folder: `vibe_code_apps_13/` (reserved).                                                    | Planned     |
| **14** | **BACnet routing research lab**                                                              | **BACpypes3** learning lab: dual [**mini-device-revisited**](https://github.com/JoelBender/BACpypes3/blob/main/samples/mini-device-revisited.py) servers on one host (ports **47809/47810**), [**ipv4-to-ipv4**](https://github.com/JoelBender/BACpypes3/blob/main/samples/ipv4-to-ipv4.py) router (nets **100/200** on **47808/47809**), **60s timed labs** with **tcpdump** pcaps, remote unicast discovery guide, tutorial toward [**Misty3**](https://github.com/raghavan97/misty3) and [**bacnet-stack router-mstp**](https://github.com/bacnet-stack/bacnet-stack/tree/master/apps/router-mstp). Folder: `vibe_code_apps_14`. | **Active**  |
| **15** | **STM32 bare-metal BACnet device (planned)**                                                 | **Future vibe app:** embedded **C** firmware on **STM32** using the **C BACnet stack** on bare metal — field-style device/controller bring-up, MS/TP or UART paths as the board allows, separate from the Pi/Linux demos.                                                                                            | Planned     |
| **16** | **Rust BACnet stack**                                                                        | **Future vibe app:** integrate [`rusty-bacnet`](https://github.com/jscott3201/rusty-bacnet) with Python bindings for performance-sensitive BACnet/IP paths, benchmarks, and optional bridge experiments alongside BACpypes3.                                                                                          | Planned     |
