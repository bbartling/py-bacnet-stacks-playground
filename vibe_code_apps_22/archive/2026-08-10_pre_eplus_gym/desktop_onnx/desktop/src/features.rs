...
/// Hourly feature builders kept for quarantined `model` / offline compare tests only.
#[cfg(test)]
mod hourly {
    use super::{STRATEGY_IDS, N_FEATURES};
...
mod tests {
    use super::hourly::*;
    use super::{default_occ_frac, scale_features, N_FEATURES};

    #[test]
    fn build_features_has_39_and_scales() {
        let h = HourInputs {
            hour_ending: 8.0,
            month: 1.0,
            doy: 15.0,
            ...
            rh_pct: 50.0,
            ghi: 100.0,
            occ_frac: default_occ_frac(8, "baseline", false),
            hp_on: [1.0; 6],
            knobs: StrategyKnobs::for_id("baseline"),
            strategy_id: "baseline".into(),
            facility_kw_lag1: 50.0,
            facility_kw_lag2: 48.0,
            hdd65_cum_night: 10.0,
        };
        let raw = build_features(&h);
        assert_eq!(raw.len(), N_FEATURES);
        let mean = [0.0_f32; N_FEATURES];
        let scale = [1.0_f32; N_FEATURES];
        let scaled = scale_features(&raw, &mean, &scale);
        assert!((scaled[0] - 8.0).abs() < 1e-5);
        let c = cost_from_hourly_kw(&[10.0; 24], 0.1, 12.0, 90.0);
        assert!((c.energy_kwh - 240.0).abs() < 1e-5);
        assert!((c.peak_kw - 10.0).abs() < 1e-5);
        assert!((c.energy_cost - 24.0).abs() < 1e-5);
        assert!((c.demand_cost - 120.0).abs() < 1e-5);
        assert!(c.total_cost > 0.0);
        assert!(c.annual_energy_stub > 0.0);
        assert!(c.annual_demand_stub > 0.0);
        assert!(c.annual_total_stub > 0.0);
    }
}
