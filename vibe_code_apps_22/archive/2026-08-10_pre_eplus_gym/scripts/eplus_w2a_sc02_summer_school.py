#!/usr/bin/env python3
"""SC02 + school-out summer dial: short Jun–Aug run hours, better cooling COP.

Research (anonymized Creekside): summer school Mon–Thu ~8–13, not full school day.
Standing Aug/Sep over-predict (~+25%) is year-round occ/equip/HVAC schedules.
"""
...
TRIALS: list[tuple[str, W2APlantKnobs, dict[str, Any]]] = [
    (
        "S05_sum025_clg46_noHvacCut",
        _sc02(
            summer_sch_scale=0.25,
            clg_coil_cop_mult=_clg_mult(4.6),
            summer_include_hvac=False,
        ),
        {"note": "soft summer: occ/plugs/lights only + cooling COP 4.6"},
    ),
    (
        "S06_sum040_clg46_noHvacCut",
        _sc02(
            summer_sch_scale=0.40,
            clg_coil_cop_mult=_clg_mult(4.6),
            summer_include_hvac=False,
        ),
        {"note": "milder summer occ/plugs 40% + cooling COP 4.6"},
    ),
    (
        "S07_r02_sum040_clg46",
        _sc02(
            summer_sch_scale=0.40,
            clg_coil_cop_mult=_clg_mult(4.6),
            summer_include_hvac=False,
        ...
        {"note": "R02 dual + soft summer polish"},
    ),
]
...
def main() -> int:
    ...
    camp = (
        ...
        / "eplus"
        / "campaigns"
        / f"w2a_sc02_summer_soft_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    )
    ...
    print("SC02 + summer-school schedules + better cooling COP", flush=True)
    ...
    for tid, knobs, meta in TRIALS:
        ...
        print(
            f"RUN {tid} summer={knobs.summer_sch_scale} "
            f"clg_mult={knobs.clg_coil_cop_mult} fields={applied['n_fields_changed']}",
            flush=True,
        )
        man = run_energyplus(
    ...
    summary = {
        ...
        "family": "W2A_sc02_summer_school",
        "hypothesis": (
            "Creekside (fictionalized) is mostly out of session Jun–Aug; "
            "cut occ/plugs/HVAC run hours + raise cooling COP 4.6–4.8."
        ),
