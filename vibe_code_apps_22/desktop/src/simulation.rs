...
mod contracts {
    ...
    pub fn result_from_series(
        ...
        let (new_billing_peak_kw, incremental_demand_kw, incremental_demand_cost) =
            super::incremental_demand(existing_billing_peak_kw, peak_kw, demand_rate_per_kw);
        SimulationResult {
            facility_kw: fac,
...
mod tests {
    use super::contracts::*;
    use super::*;
    use crate::control_contract::load_control_schedule;
    use crate::features_15min::STEPS_96;
    ...
    #[test]
    fn simulation_request_and_result_smoke() {
        let oat = [20.0_f32; STEPS_96];
        let req = SimulationRequest::from_strategy_id(
            "baseline",
            40.0,
            [68.0; 6],
            oat,
        ...
        .expect("request");
        assert_eq!(req.strategy_id(), "baseline");
        assert!((req.midnight_facility_kw - 40.0).abs() < 1e-6);
        assert!((req.midnight_zone_temps_f[0] - 68.0).abs() < 1e-6);
        assert!((req.oat_f_96[0] - 20.0).abs() < 1e-6);
        assert!((req.rh_pct_96[0] - 55.0).abs() < 1e-6);
        assert!(req.ghi_96.iter().any(|v| *v > 0.0));
        assert!((req.month - 1.0).abs() < 1e-6);
        assert!((req.doy - 15.0).abs() < 1e-6);
        assert!(!req.is_weekend);
        assert_eq!(req.schedule.steps.len(), STEPS_96);
        assert!(!req.tariff.label.is_empty());
        assert!((req.existing_billing_peak_kw - 100.0).abs() < 1e-6);
        assert!(req.billed_demand_kw.is_none());
        assert!(!req.honesty_note.is_empty());
        let fac = vec![50.0_f64; STEPS_96];
        let zones = vec![[68.0_f64; 6]; STEPS_96];
        let res = result_from_series(
            &fac,
            &zones,
            "baseline",
            100.0,
        ...
        assert!((res.peak_kw - 50.0).abs() < 1e-9);
        assert!((res.facility_kw[0] - 50.0).abs() < 1e-6);
        assert!((res.zone_temperatures_f[0][0] - 68.0).abs() < 1e-6);
        assert!(res.daily_kwh > 0.0);
        assert_eq!(res.peak_timestep, STEPS_96 - 1);
        assert!((res.energy_cost - 10.0).abs() < 1e-9);
        assert!(res.incremental_demand_kw.abs() < 1e-9);
        assert!(res.incremental_demand_cost.abs() < 1e-9);
        assert!((res.new_billing_peak_kw - 100.0).abs() < 1e-9);
        assert_eq!(res.comfort_violations, 0);
        assert!(!res.ood);
        assert!(res.ood_status.is_none());
        assert!(res.recommend);
        assert_eq!(res.honesty, "test");
        assert_eq!(res.provenance, "unit");
        assert_eq!(res.strategy_id, "baseline");
        assert!(res.unsupported_reason.is_none());
        assert_eq!(res.outcome_flags, vec!["ok".to_string()]);
        assert!(!future_objective_doc().is_empty());
        assert_eq!(zone_col_names().len(), 6);
    }
}
