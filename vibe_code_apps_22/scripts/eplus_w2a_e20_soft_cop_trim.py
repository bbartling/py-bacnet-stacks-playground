#!/usr/bin/env python3
"""Soft COP trim on E20: rated heating COP 4.0 / 4.5 / 4.7 (vs E20's 5.04).

Year-round rated dial only — no seasonal COP schedule in W2A knobs.
Goal: nudge Jan-26 peak toward ~285 while watching monthly GL14.
...
BASE_RATED_COP = 4.2
OVERNIGHT_MAX_KW = 145.0  # E20 overnight was ~135; allow a little headroom
...
TRIALS: list[tuple[str, W2APlantKnobs, dict[str, Any]]] = [
    (
        "SC01_cop47",
        _e20(htg_coil_cop_mult=_cop_mult(4.7)),
        {"target_rated_cop": 4.7, "note": "slight trim from E20 5.04"},
    ),
    (
        "SC02_cop45",
        _e20(htg_coil_cop_mult=_cop_mult(4.5)),
        {"target_rated_cop": 4.5, "note": "user-requested 4.5"},
    ),
    (
        "SC03_cop40",
        _e20(htg_coil_cop_mult=_cop_mult(4.0)),
        {"target_rated_cop": 4.0, "note": "user-requested 4.0"},
    ),
]
...
def main() -> int:
    ...
    camp = (
        ...
        / "eplus"
        / "campaigns"
        / f"w2a_e20_soft_cop_trim_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    )
    ...
    print("E20 soft COP trim (year-round rated dial): 4.7 / 4.5 / 4.0 vs E20 5.04", flush=True)
    ...
    for tid, knobs, meta in TRIALS:
        ...
        print(
            ...
            f"eff_rated≈{rated} fields={applied['n_fields_changed']}",
            flush=True,
        )
        man = run_energyplus(
    ...
    summary = {
        ...
        "family": "W2A_e20_soft_cop_trim",
        "note": (
            "Year-round rated heating COP trim on E20 knobs; "
            "not a winter-only schedule (W2A has no seasonal COP dial)."
        ),
        "e20_anchor": {"peak_kw": 270.8, "rated_cop": 5.04, "gl14": True},
