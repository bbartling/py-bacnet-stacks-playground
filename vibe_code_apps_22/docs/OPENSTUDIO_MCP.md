# OpenStudio MCP bridge (optional)

Lives under `vibe_code_apps_22/openstudio_mcp_bridge/`. Not required for IdealLoads
G14 or heating DSM — native OpenStudio SDK on `LAKESIDE_SITE_ROOT` is enough.
...
3. Site models under `VIBE23_LAKESIDE_ROOT` (IDF/EPW/OSM)
...
| Host | Container |
| --- | --- |
| `VIBE23_LAKESIDE_ROOT\eplus\models` | `/inputs` |
| `vibe_code_apps_22\openstudio_mcp_bridge\runs` | `/runs` |
| `vibe_code_apps_22\openstudio_mcp_bridge\measures` | `/measures` |
...
```powershell
python -u $env:VIBE23_LAKESIDE_ROOT\scripts\eplus_campaign_utility.py
```
