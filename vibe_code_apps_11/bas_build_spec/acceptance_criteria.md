...
- [x] Application starts with Docker Compose using documented commands
...
- [x] Domain models: site, building, floor, equipment, point, trend sample, alarm, schedule, user/role, audit log
- [x] Business logic not confined to route handlers
...
- [x] Equipment pages show live point values
...
- [x] Usable on desktop without developer tools
...
- [x] **Building-program templates:** documented way to represent multiple archetypes (examples: **VAV + central AHU**, **VRF + DOAS**, **HP DOAS**, **data center CRAH / CHW**, **lab** ACH or pressurization context, **hospital** isolation OR / critical zones, **lighting-only** or mixed-use). Seeded or config-driven **equipment + point sets** match the chosen template(s) for the demo site.
- [x] **Primary HVAC / plant graphic(s):** at least one synoptic for the site’s *primary* plant or air side (e.g. central plant, DOAS, VRF header, CRAH row—whatever the template defines) with **live BACnet-backed or simulated** points appropriate to that type.
- [x] **Terminal / zone graphic:** at least one graphic for a **representative terminal or zone** (e.g. VAV box, VRF indoor unit, fan coil, room/zone summary) appropriate to the template—not necessarily “VAV” in the name if the building is VRF + DOAS.
- [x] **Secondary / ancillary graphic:** at least one additional system view when the program warrants it (e.g. lighting, exhaust, reheat, waterside, or a **generic** equipment page) so multi-system campuses are representable.
- [x] Live values update **without full page refresh** (SSE/WebSocket or equivalent).
- [x] Graphics **deep-link** to properties, trends, schedules, and alarms for the equipment/points shown.
- [x] **Authorized users** can start the **safe setpoint/command** workflow from a graphic (same rules as Command/write section).
...
- [x] Metadata edits restricted by role
- [x] Status shows normal, alarm, stale, offline, overridden, disabled, fault as applicable
...
- [x] Multi-point graph, date range selection
- [x] Chart shows historical values
- [x] CSV export
- [x] Samples include timestamp, value, quality/status
- [x] Intervals configurable or modeled
...
- [x] Config edits logged
- [x] Audit filterable and CSV export

## Security

- [x] Auth required for app access
...
- [x] Reasonable CORS/defaults for local dev
...
- [x] Start/stop/config via env or documented command
...
- [x] Frontend or E2E: login, navigation, alarm ack, schedule edit, command flow (where practical)
- [x] Documented test command; no unexplained failing tests
- [x] Lint/format documented

## Documentation

- [x] README: purpose, local run, Compose, demo users, simulator, safety, future BACnet path
- [x] Acceptance criteria honestly marked complete/incomplete

### Codex / operator hint (not checklist)

LAN dial-in triage belongs in **`bas_app/README.md`** (ports **5173/8000**, **`BAS_ALLOWED_ORIGINS`**, firewall) and optionally **`memory/architecture/working-divergence.md`**. Do **not** add new **`[ ]`** acceptance rows to “re-open” a signed-off checklist unless the human explicitly changes scope.
...
## Roadmap — commissioning phases (incremental; does not revoke release gate above)

Aligned with **`spec.md`** § *Commissioning and construction phases* and **`skills/field-commissioning-phases`**. Check **`[x]`** only when verified like other criteria; until then **`[ ]`** is expected.

**Backlog (intentionally open):** The **six** `- [ ]` rows below define **commissioning work**—including the **no-login electrician / paste** surface and the **Day 0 unified shell**—and stay unchecked until you **field-verify** and flip them. **`cron_codex/bin/check_acceptance_complete.sh`** treats **any** `- [ ]` in this file as incomplete (so turning on **`REMOVE_CRON_WHEN_COMPLETE=true`** will not declare “all done” until these are checked or removed). **§ Security “Auth required for app access”** still applies to the **operator shell**; **public rough-in URLs** are **narrow, documented exceptions** per **`spec.md`**, not a global removal of auth.
- [ ] **Phase 1 — Electrician (hardware / rough-in):** **No login** on **`/rough-in/`**; **chat-first** commissioning dialog (HVAC system type, expected BACnet devices, ongoing messages) with **persisted conversation** referenced across wakes; **bare-bones tables** for BACnet status, networking (**5173/8000/47808**), and read-only device/point values; **minimal color** + **dark/light mode** toggle; **no writes** on public routes; operator login shell unchanged at **`/`**. LAN + CORS documented.
- [ ] **Day 0 unified commissioning shell (UX):** Same **chat + persisted thread** on all commissioning phases (Phase 1 public first); **simple tables** not dense card dashboards; **dark/light** on Phases 1–3; technician/TAB may add capabilities behind login later but **reuse the same chat history**; Phase 4 full supervisor remains separate URL with login.
- [ ] **Phase 2 — Cx + HVAC point-to-point:** Writable **point adjustments** with reason + audit; **functional / P2P** commissioning flows across equipment; distinct mode vs Phase 1.
- [ ] **Phase 3 — TAB:** **Functional testing & balancing**; **dropdown (or multi-select) chart builder** for **sensor trend overlays**; exports / witness artifacts.
- [ ] **Phase 4 — Final BAS (owner):** Full password-protected supervisor (regression target for existing demo).
- [ ] **Phase completion channel:** Notepad v1 in git; optional **TUI/CLI** timestamped logger only when **`BUILD_CHECKPOINTS`** schedules it.
