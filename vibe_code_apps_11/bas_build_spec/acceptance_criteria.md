...
- [ ] **Phase 1 — Electrician (hardware / rough-in):** Read-only dial-in: **BACnet driver / stack status** (healthy vs simulator vs error; bind/discovery state when lab on); **Networking** tab (**NIC** list, IPv4, up/down, **listener summary** for BAS ports, **documented** expected **UFW** holes e.g. **5173/tcp**, **8000/tcp**, **47808/udp**—align with ops, no secrets in UI); **all readable sensors / present values** + **online/offline**; **no** writes; LAN + CORS documented; **`PHASE_NOTEPAD`** used for paste-in context.
- [ ] **Day 0 unified commissioning shell (UX):** Single dial-in shell with **notepad/chat + Save**, **always-on phase/BACnet/URL strip**, **read-only telemetry** after context saved, **roadmap checkboxes** + links to **`bas_validate_*`** / wake **log** affordances (per `vibe_code_app_11_notes.txt` § 3–4), **electrician / technician / TAB** zones per **`spec.md`** unified shell paragraph—**dark BAS** chrome (`graphic.html`).
