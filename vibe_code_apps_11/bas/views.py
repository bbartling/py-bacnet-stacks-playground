...
def bas_manage_users(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            np.phone_number = str(request.POST.get('phone_number') or '').strip()
            np.save()
        ...
        if action == 'update_profile':
            ...
            # update phone number
            vp.phone_number = str(request.POST.get('phone_number') or '').strip()
            vp.save()
            if me.is_superuser:
...
#
# These APIs provide filtered lists of points or devices based on status
# conditions. The logic mirrors Niagara BQL queries like:
#
#   select name, out.value from control:ControlPoint where status.down = 'true'
#
# but implemented using Django ORM filters to avoid SQL injection. Clients
# supply a ``status`` query parameter (e.g. ``down``, ``disabled``,
# ``overridden``, ``stale``, ``fault``) and the server returns matching
# objects. Only authenticated users can access these endpoints.

@_require_auth
@require_http_methods(["GET"])
def api_status_points(request: HttpRequest) -> JsonResponse:
    """Return points filtered by a status query parameter.

    Allowed statuses:
    - ``down``: the associated device is offline.
    - ``disabled``: ``Point.disabled`` is true.
    - ``overridden``: ``Point.overridden`` is true.
    - ``stale``: ``Point.value_state`` equals 'stale'.
    - ``fault``: ``Point.fault`` is true.

    If no status parameter is provided, all points are returned. Results are
    returned as a JSON object with a single ``items`` list. Each item
    includes ``pointId``, ``label``, ``deviceInstance`` and the statuses as
    booleans to allow the frontend to colour code rows.
    """
    from bas.models import Point  # import locally to avoid circular import
    status = str(request.GET.get('status') or '').strip().lower()
    allowed = {'', 'down', 'disabled', 'overridden', 'stale', 'fault'}
    if status not in allowed:
        return JsonResponse({'detail': 'invalid status'}, status=400)
    qs = Point.objects.all()
    if status == 'down':
        qs = qs.filter(device__offline=True)
    elif status == 'disabled':
        qs = qs.filter(disabled=True)
    elif status == 'overridden':
        qs = qs.filter(overridden=True)
    elif status == 'stale':
        qs = qs.filter(value_state='stale')
    elif status == 'fault':
        qs = qs.filter(fault=True)
    items = []
    for p in qs:
        items.append({
            'pointId': p.point_id,
            'label': p.label or p.point_id,
            'deviceInstance': p.device.device_instance if p.device else None,
            'disabled': bool(getattr(p, 'disabled', False)),
            'overridden': bool(getattr(p, 'overridden', False)),
            'fault': bool(getattr(p, 'fault', False)),
            'stale': (p.value_state == 'stale'),
            'down': bool(getattr(p.device, 'offline', False)) if p.device else False,
        })
...
@_require_auth
@require_http_methods(["GET"])
def api_status_devices(request: HttpRequest) -> JsonResponse:
    """Return devices filtered by a status query parameter.

    Supported statuses:
    - ``down``: ``Device.offline`` is true.
    - ``fault``: ``Device.status`` contains the word 'fault' (case‑insensitive).

    If no status is supplied, all devices are returned. Results are
    returned as ``items`` containing basic device metadata.
    """
    from bas.models import Device  # import locally to avoid circular import
    status = str(request.GET.get('status') or '').strip().lower()
    allowed = {'', 'down', 'fault'}
    if status not in allowed:
        return JsonResponse({'detail': 'invalid status'}, status=400)
    ...
    if status == 'down':
        qs = qs.filter(offline=True)
    elif status == 'fault':
        qs = qs.filter(status__icontains='fault')
    items = []
    for d in qs:
        items.append({
            'deviceInstance': d.device_instance,
            'label': d.label or f'Device {d.device_instance}',
            'status': d.status,
            'offline': d.offline,
            'note': d.note,
        })
