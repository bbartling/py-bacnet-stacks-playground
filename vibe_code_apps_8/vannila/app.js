(function () {
  ...
  const init = initialSchedules();
  const now = new Date();
  const state = {
    schedules: init.list,
    activeScheduleId: init.activeId,
    holidays: [],
  ...
  function syncOperatingRowVisual(day) {
    const profile = getActiveProfile();
    const rowEl = document.querySelector(`#operating-table .form-row[data-day="${day}"]`);
    if (!rowEl) return;
  ...
  function onBacnetAdd() {
    const n = document.getElementById('bacnet-new-name');
    const o = document.getElementById('bacnet-new-oid');
  ...
  function syncOperatingHint() {
    const el = document.getElementById('operating-hint');
    const p = getActiveProfile();
    if (el) {
  ...
  function syncBacnetHint() {
    const el = document.getElementById('bacnet-hint');
    const p = getActiveProfile();
    if (el) {
  ...
  function syncOperatingValues() {
    const profile = getActiveProfile();
    for (const day of DAYS) {
      const row = document.querySelector(`#operating-table .form-row[data-day="${day}"]`);
      if (!row) continue;
