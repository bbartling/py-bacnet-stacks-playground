...
export function BacnetPointsEditor({ points, onChange, profileName }: Props) {
  ...
  return (
    <section className="panel" aria-labelledby="bacnet-heading">
      ...
      <p className="section-hint">
        {profileName ? (
          <>
            Points assigned to the <strong>{profileName}</strong> schedule only (switch schedule above to edit another
            profile&apos;s list). Object ID is optional metadata for integration (e.g. AV:1).
          </>
        ) : (
          <>Configure supervisory points. Object ID is optional metadata for integration (e.g. AV:1).</>
        )}
      </p>
        ...
        {points.map((p) => (
          <div key={p.id} className="bacnet-row">
            ...
            <div className="bacnet-actions">
              <button type="button" className="btn danger" onClick={() => removePoint(p.id)}>
                Remove
              </button>
            </div>
          </div>
