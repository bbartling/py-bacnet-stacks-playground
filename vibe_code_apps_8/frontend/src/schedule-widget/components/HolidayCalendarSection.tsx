...
import type { HolidayEntry } from "../shared/scheduleTypes";
  ...
  return (
    <section className="panel holiday-panel" aria-labelledby="holiday-heading">
      ...
      <p className="section-hint">
        ...
        <strong> Date range:</strong> click the start day, then the end day (like sliding a span), then add — every day in
        between is included. New entries default <strong>unoccupied</strong> unchecked; use Delete to remove.
      </p>

      <div className="holiday-mode-toggle" role="group" aria-label="Holiday selection mode">
        <button type="button" className={`btn mode-btn ${pickerMode === "multiple" ? "active" : ""}`} onClick={() => setMode("multiple")}>
          Individual dates
        </button>
        <button type="button" className={`btn mode-btn ${pickerMode === "range" ? "active" : ""}`} onClick={() => setMode("range")}>
          Date range
        </button>
      </div>
