...
export function HolidayCalendarSection({ holidays, onChange }: Props) {
  ...
  function setUnoccupied(id: string, value: boolean) {
    onChange(
      holidays.map((h) => (h.id === id ? { ...h, unoccupied: value } : h))
    );
  }
        ...
        <div className="holiday-picker-wrap">
          ...
          {pickerMode === 'multiple' ? (
            <button
              ...
              onClick={addMultipleToHolidays}
              disabled={!pickerDates?.length}
            >
            ...
            <>
              ...
              {rangePartial && (
                <p className="holiday-range-hint">
                  Second click sets the end of the range. Or use the button above
                  to add just the first day.
                </p>
              )}
            </>
          )}
              ...
              {holidays.map((h) => (
                <li key={h.id} className="holiday-list-item">
                  ...
                  <label className="holiday-occ">
                    <input
                      type="checkbox"
                      checked={h.unoccupied}
                      onChange={(e) =>
                        setUnoccupied(h.id, e.target.checked)
                      }
                    />
                    Unoccupied
                  </label>
