#!/usr/bin/env python3
"""
Build a single PDF from Markdown under a docs tree (Jekyll-style front matter optional).
...
Usage:
  ...
  python3 scripts/build_docs_pdf.py --docs-dir vibe_code_apps_8/docs \\
      --title \"BAS Lite (App 8)\" -o vibe_code_apps_8/documentation.pdf
...
# Repo layout: script in scripts/, default docs in docs/, default output in pdf/
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build Markdown docs into a single PDF (Pandoc + weasyprint or pdflatex)."
    )
    parser.add_argument(
        "--docs-dir",
        type=Path,
        default=None,
        help=f"Directory to scan for *.md (default: {DEFAULT_DOCS_DIR})",
    )
    parser.add_argument(
        "--title",
    ...
    parser.add_argument(
        "--keep-md",
        action="store_true",
        help="Keep combined Markdown in <docs-dir>/_build/combined.md",
    )
    parser.add_argument(
        "--pdf-engine",
    ...
    docs_dir = (args.docs_dir or DEFAULT_DOCS_DIR).resolve()
    ...
    try:
        subprocess.run(cmd, check=True, cwd=docs_dir.parent, env=env)
